...
class ProfileSoraWM(SoraWM):
    def run(
        ...
        """
        Execute watermark detection and removal on the input video and write a cleaned video with the original audio to the specified output path.
        
        Performs per-frame watermark detection, fills missing detections, processes the video in breakpoint-defined segments using the configured cleaner, encodes an intermediate video, and merges the original audio into the final output.
        ...
        Parameters:
            ...
            progress_callback (Callable[[int], None] | None): Optional callback invoked with an integer progress percentage (0–100) at key points during processing.
            quiet (bool): If True, suppresses progress bars and most debug logging.
        """
