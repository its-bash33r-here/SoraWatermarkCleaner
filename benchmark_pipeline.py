...
def benchmark_method(
    ...
    try:
        ...
        if method_name == "run":
            method(input_path, output_path, overlap_running=overlap_running)
        else:
...
def main():
    ...
    time_parallel = benchmark_method(
        "run", sora_wm, input_video_path, output_parallel, overlap_running=True
    )
    time.sleep(2)

    # Test 1: Serial execution (original method)
    time_serial = benchmark_method(
        "run", sora_wm, input_video_path, output_serial, overlap_running=False
    )
    ...
    if time_serial and time_parallel:
        ...
        logger.info(f"Pipeline parallel (run):  {time_parallel:.2f} seconds")
        logger.info("")
